...
export default definePlugin({
  ...
  capabilities: {
    marks: [mark('subscript', { tag: 'sub', parseTags: ['sub'], priority: 3 })],
    commands: {
      subscript: command((state, registries) => {
        const s = forceRemoveMark(state, registries, 'superscript')
        return toggleMark(s, registries, 'subscript')
      }),
    },
    toolbar: [
      toolbarItem({
        id: 'subscript',
        label: 'subscript.button',
        icon: ICON,
        shortcut: 'mod+=',
        activeMark: 'subscript',
        group: 'format',
        order: 40,
      }),
    ],
    shortcuts: shortcut('mod+=', 'subscript'),
  },
})
